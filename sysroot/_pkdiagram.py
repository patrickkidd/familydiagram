...
MODULE_DIR = os.path.realpath(os.path.join(os.path.dirname(__file__), '..', 'pkdiagram', '_pkdiagram'))
...
class PKDiagramComponent(Component):
    ...
    def install(self):
        orig_cwd = os.getcwd()
        try:
            _path = os.path.realpath(os.path.join(self._sysroot.sysroot_dir, '..', '..', 'pkdiagram', '_pkdiagram'))
            os.chdir(_path)
            self.run('sip-build')
