...
class PKDiagramComponent(Component):
    ...
    def verify(self):
        # Only verify the non-generated source files
        for fpath in [os.path.join(MODULE_DIR, x) for x in SOURCES]:
            if not os.path.isfile(fpath):
                self.error(f"The source file doesn't exist: {fpath}")

        # BUILD_SOURCES are generated by sip-build during install(), so don't verify them here
