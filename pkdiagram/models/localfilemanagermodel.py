...
class LocalFileManagerModel(FileManagerModel):
    ...
    def onFileAdded(self, url, status):
        ...
        if fileInfo.exists(url.toLocalFile()): # files don't exist when not downloaded yet
            modified = os.stat(url.toLocalFile()).st_mtime
        else:
            modified = None
        self.addFileEntry(url.toLocalFile(),
    ...
    def deleteFileAtRow(self, row):
        ...
        if count:
            btn = QMessageBox.question(QApplication.activeWindow(),
                                       'Are you sure?',
                                       'This diagram contains %i files, are you sure you want to delete it? This will delete it from the hard drive and cannot be undone.' % count)
        else:
