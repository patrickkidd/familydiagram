...
class EmotionPropertiesModel(QObject, ModelHelper):
    ...
    PROPERTIES = objects.Item.adjustedClassProperties(objects.Emotion, [
    ...
    def get(self, attr):
        ...
        if attr == 'kindIndex':
            ...
            ret = util.sameOf(self._items, lambda item: item.personA() and item.personA().id or -1)
        elif attr == 'personBId':
            ret = util.sameOf(self._items, lambda item: item.personB() and item.personB().id or -1)
        elif attr == 'dyadic':
