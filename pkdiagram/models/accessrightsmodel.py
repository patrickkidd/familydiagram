...
class AccessRightsModel(QAbstractListModel):
    ...
    def roleNames(self):
        return {self.UsernameRole: b"username", self.RightRole: b"rightCode"}
    ...
    def data(self, index, role=Qt.DisplayRole):
        ...
        if self._diagram:
            ...
            if index.column() == 0 and role in (Qt.DisplayRole, self.UsernameRole):
                user = util.find(
                    self._session.users, lambda x: x.id == access_right.user_id
                )
                if user:
                    ret = user.username
    ...
    def setData(self, index, value, role=Qt.EditRole):
        ...
        if index.column() == 1 or role == self.RightRole:
            if value and access_right.right != value:
                self._session.server().blockingRequest(
                    "PATCH",
                    f"/access_rights/{access_right.id}",
                    data={"right": value, "session": self._session.token},
                )
    ...
    def findUser(self, username):
        ...
        if self._session:
            ret = util.find(
                ...
                and x.username != self._session.user.username,
            )
        else:
            ret = None
        return ret
            ...
            for access_right in self._diagram.access_rights:
                if access_right.user_id == user.id:
                    QMessageBox.warning(
                        None,
                        "Could not add access right.",
                        f"An access right for {username} already exists.",
                    )
                    return False
            if (
                ...
                QMessageBox.warning(
                    None,
                    "Could not add access right.",
                    self.S_CLIENT_ONLY_ALLOWED_ONE_RIGHT,
                )
                return False
    ...
    def deleteRight(self, row):
        ...
        response = self._session.server().blockingRequest(
            "DELETE",
            f"/access_rights/{access_right.id}",
            data={"session": self._session.token},
        )
