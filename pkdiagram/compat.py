...
def update_data(data):
    ...
    if UP_TO(data, '1.0.0a16'):
        ...
        for chunk in data.get('items', []):
            if chunk['kind'] in ('Person', 'Marriage'):
                ...
                elif not 'itemPos' in chunk['detailsText'] and 'nonLayerPos' in chunk['detailsText']:
                    chunk['detailsText']['itemPos'] = chunk['detailsText']['nonLayerPos']
                ...
                if 'separationIndicator' in chunk and chunk['separationIndicator']['itemPos'].x() > 100000:
                ...
                if not 'childOf' in chunk:
                    if chunk.get('parents') is not None:
                        ...
                        if chunk.get('birthPartners'):
                            chunk['childOf']['multipleBirth'] = getMultipleBirthId(chunk['parents'], [chunk['id']] + chunk['birthPartners'])
                    else:
                        chunk['childOf'] = {}
        ...
        for chunk in data.get('items', []):
            ...
            if chunk['kind'] == 'Person':
                ...
                if birthEvent:
                    ...
                    birthEvent['dateTime'] = QDateTime(birthDate if birthDate else QDateTime())

                adoptedEvent = chunk.get('adoptedEvent')
                if adoptedEvent:
                    adoptedDate = adoptedEvent.pop('date', None)
                    adoptedEvent['dateTime'] = QDateTime(adoptedDate if adoptedDate else QDateTime())

                deathEvent = chunk.get('deathEvent')
                if deathEvent:
                    deceasedDate = deathEvent.pop('date', None)
                    deathEvent['dateTime'] = QDateTime(deceasedDate if deceasedDate else QDateTime())
    ...
    if UP_TO(data, '1.4.5'):
        ...
        for chunk in data.get('items', []):
            if chunk['kind'] in objects.Emotion.kindSlugs():
                itemTags = sorted(list(set(chunk.get('tags', []) + chunk['startEvent'].get('tags', []) + chunk['endEvent'].get('tags', []))))
