...
class PanZoomer(QObject):
    ...
    def test(self, touchPoints, zoomSceneStartScale, panSceneStartCenter):
        if not self._beganTest:  # init
            ...
            self.startMouseScenePos = self.view.mapToScene(
        ...
        if not self.begun() and not self.crossedThreshold:  # test
            ...
            if not util.ENABLE_PINCH_PAN_ZOOM:
                if (
                    not self.crossedPanThreshold
                ):  # prioritize wheel-panning by preventing every starting if panning
                    crossed = self.crossedZoomThreshold
            else:
                crossed = (
                    point0Distance >= self.ZOOM_THRESHOLD
                    and point1Distance >= self.ZOOM_THRESHOLD
                ) and (self.crossedZoomThreshold or self.crossedPanThreshold)
            return crossed
        return True
    ...
    def update(self, touchPoints):
        ...
        if util.IS_IOS:
            ...
            panSpeed = 4.0
        panXScale = (
            (panTouchCenter.x() - self.panTouchStartCenter.x()) * panSpeed
        ) / self.view.scene().scaleFactor()
        panYScale = (
            (panTouchCenter.y() - self.panTouchStartCenter.y()) * panSpeed
        ) / self.view.scene().scaleFactor()
        self.panSceneCenter = QPointF(
            self.panSceneStartCenter.x() - panXScale,
    ...
    def doPan(self, sceneCenter=None):
        if util.ENABLE_PINCH_PAN_ZOOM:  # TODO: rename to ENABLE_PINCH_PAN_ZOOM
            ...
            currentMouseScenePos = self.view.mapToScene(
                self.view.mapFromGlobal(QCursor.pos())
            )
    ...
    def timerEvent(self, e):
        ...
        if self.crossedThreshold and not self.finishedCatchUp:
            ...
            if elapsed < self.ANIM_DURATION_MS:
                ...
                # scale
                zoomScaleDeltaTotal = (
                    self.zoomSceneScaleFactor - self.zoomSceneStartScale
                )
                ...
                panSceneCenter = (
                    self.panSceneStartCenter + panSceneDeltaTotal * keyFrameCoeff
                )
