"""
FD-122: This needs to go in pkdiagram.app, not pkdiagram.scene
"""
...
from pkdiagram.scene import Person, Callout, Marriage, ChildOf, LayerItem
...
class MoveItems(UndoCommand):
    ...
    def redo(self):
        if self.firstRun:
            ...
            for item, entry in self.data.items():
                ...
                if isinstance(item, Callout) and entry["calloutPoints"]:
    ...
    def undo(self):
        for item, entry in self.data.items():
            ...
            if isinstance(item, Callout) and entry["calloutPoints_was"]:
...
class Clipboard:
    ...
    def copy(self, scene):
        ...
        for item in self.items:
            ...
            clones.append(clone)
            if isinstance(clone, Person) and not item.childOf in self.items:
        ...
        people = []
        for clone in clones:
            if isinstance(clone, Person):
                people.append(clone)
            elif isinstance(clone, ChildOf):
