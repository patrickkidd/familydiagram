...
from pkdiagram.pyqt import QApplication, QTimer
...
def test_autosave_manager_creates_folder(tmp_path, qApp):
    ...
    with mock.patch(
        "_pkdiagram.CUtil.instance().documentsFolderPath", return_value=str(tmp_path)
    ):
...
def test_autosave_on_document_open(tmp_path, create_ac_mw):
    ...
    # Verify the autosaved file contains the correct data
    ...
    saved_scene = Scene()
    saved_scene.read(saved_data)
    assert saved_scene.query1(name="Test Person")
...
def test_autosave_stops_when_document_closed(tmp_path, create_ac_mw):
    ...
    assert mw.autoSaveManager._timer.isActive()
    ...
    assert not mw.autoSaveManager._timer.isActive()
...
def test_autosave_periodic_save(tmp_path, create_ac_mw, qtbot):
    ...
    # Manually trigger timer timeout to simulate periodic save
    mw.autoSaveManager._onTimerTimeout()
    QApplication.instance().processEvents()
...
def test_autosave_readonly_document(tmp_path, create_ac_mw):
    ...
    mw.autoSaveManager.autoSaved.connect(lambda path: autosave_count.__setitem__(0, autosave_count[0] + 1))
