...
class Drawer(QFrame):
    ...
    def __init__(self, parent=None, resizable=False):
        ...
        self.ideallyResizable = resizable # should resize, but temporarily not resizable (i.e. expanded)
    ...
    def setupResizable(self):
                ...
                if e.type() == QEvent.Enter:
                    ...
                    if self.mouseDown:
                        ...
                        if not self.parent().expanded:
                            newWidth = max(self.mouseDownWidth - delta, self.minimumWidth)
                            newWidth = min(newWidth, self.parent().parent().width())
                            self.parent().resize(newWidth,
                                                 self.parent().height())
                            maxX = self.parent().parent().width() - self.parent().width()
        ...
        if self.parent():
            self.resize(self.WIDTH, self.parent().height()) # minimumWidth was being set to designer width. no idea.
        self.resizable = True
    ...
    def paintEvent(self, e):
        ...
        if not self.resizable or self._lockResizeHandle:
            ...
            if util.CUtil.instance().isUIDarkMode():
                if util.IS_IOS:
                    ...
                    handleColor = QApplication.instance().palette().color(QPalette.Button)
                handleEdgeColor = Qt.black
            else:
                handleColor = QColor(Qt.lightGray).lighter(130)
                handleEdgeColor = borderColor
            p.fillRect(QRect(0, 0, util.RESIZE_HANDLE_WIDTH, self.height()), handleColor)
            ...
            if e.rect().intersects(insideLeftRect):
                ...
                p.drawLine(0, 0, 0, self.height()) # insideLeftRect.topLeft(), insideLeftRect.bottomLeft())
                p.setPen(QPen(handleEdgeColor, 0))
                p.drawLine(util.RESIZE_HANDLE_WIDTH, 0, util.RESIZE_HANDLE_WIDTH, self.height()) # insideLeftRect.topLeft(), insideLeftRect.bottomLeft())
                p.restore()
        ...
        if e.region().intersects(outsideRightRect):
            ...
            p.drawLine(outsideRightRect.topRight(),
                       outsideRightRect.bottomRight())
            p.restore()
        p = None
    ...
    def hide(self, callback=None, animate=True, swapping=False):
        ...
        if animate:
            ...
            self.expandAnimation.setDuration(util.ANIM_DURATION_MS * .75)
        else:
            self.expandAnimation.setDuration(0)
        if callback:
            def doCallback():
    ...
    def updatePos(self):
        ...
        if self.showing:
            travelX = (self.parent().width() - self.width()) - self._expandAnimStartGeo.x()
            travelW = 0
        elif self.hiding:
    ...
    def onExpandAnimationStart(self):
        ...
        if self.documentView():
            if self.showing:
                ...
                action = 'hiding'
            self.documentView().onDrawerAnimationStart(self, action, self.expandAnimation.currentValue())
    ...
    def onExpandAnimationTick(self, x):
        ...
        if self.expandAnimation.state() == QAbstractAnimation.Running:
            self.updatePos()
            if self.documentView():
                progress = self.expandAnimation.currentTime() / self.expandAnimation.totalDuration()
                if self.hiding:
                    if progress == 0:
                        progress = 1.0
                    else:
                        progress = 1 - progress
                self.documentView().onDrawerAnimationTick(self, self.expandAnimation.currentValue())

    def onExpandAnimationFinished(self):
        self.updatePos()
        if self.documentView():
            self.documentView().onDrawerAnimationFinished(self, self.expandAnimation.currentValue())
        if self.showing:
            self.showing = False
            self.shown = True
