...
def quickWidget(item: QQuickItem) -> QQuickWidget:
...
def focus(item: Union[QQuickItem, str]):
    view = quickWidget(item)
    if not view.isActiveWindow():
        # self.here('Setting active window to %s, currently %s' % (self, QApplication.activeWindow()))
        QApplication.setActiveWindow(view)
        log.debug(f'focus("{item.objectName()}")')
        QTest.qWaitForWindowActive(self, 5000)
        if not view.isActiveWindow():
            raise RuntimeError(
                "Could not set activeWindow to %s, currently is %s"
                % (view, QApplication.activeWindow())
            )
        # else:
        #     Debug('Success setting active window to', self)
    assert (
    ...
    log.debug(f'QmlWidgetHelper.focusItem("{item.objectName()}")')
    ...
    if not item.hasActiveFocus():
        ...
        if not view.isActiveWindow():
            ...
            msg += "\n    - root item size: %s, %s" % (
                view.rootObject().property("width"),
                view.rootObject().property("height"),
            )
            raise RuntimeError(msg)
    return item


def _resetFocus(item: QQuickItem):
    view = quickWidget(item)
    log.debug(f"QmlWidgetHelper.resetFocus({itemString(item)})")
    item.setProperty("focus", False)
    if item.hasActiveFocus():
        view.rootObject().forceActiveFocus()  # TextField?
        if item.hasActiveFocus():
            raise RuntimeError("Could not re-set active focus.")


def mouseClick(item: QQuickItem, button=Qt.MouseButton.LeftButton, pos=None):
    view = quickWidget(item)
    if pos is None:
        rect = item.mapRectToScene(
            QRectF(0, 0, item.property("width"), item.property("height"))
    ...
    QTest.mouseClick(view, button, Qt.KeyboardModifier.NoModifier, pos)


def keyClicks(item: QQuickItem, s: str, resetFocus=True, returnToFinish=True):
    view = quickWidget(item)
    focus(item)
    log.debug(
        f'QmlWidgetHelper.keyClicks("{item.objectName()}", "{s}", resetFocus={resetFocus}, returnToFinish={returnToFinish})'
    )
    QTest.keyClicks(view, s)
    if returnToFinish:
        log.debug(
            f'QmlWidgetHelper.keyClicks[returnToFinish]("{item.objectName()}", {s})'
        )
        QTest.keyClick(view, Qt.Key_Return)  # only for TextInput?
    if resetFocus:
        _resetFocus(item)
    QApplication.processEvents()
