...
if not IS_DEV:
    ...
    SERVER_URL_ROOT = 'http://127.0.0.1:10000' # so it doesn't connect to the dev server on 8888
else: # IS_DEV
    if IS_WINDOWS:
...
SAVE_FILE_TYPES = "Family Diagram (*.%s);;Image JPEG (*.jpg *.jpeg);;Image PNG (*.png);;Excel (*.xlsx)" % EXTENSION
...
BORDER_RADIUS = 5
#
DEFAULT_USE_TIME = QDateTime(QDate(2000, 1, 1), QTime(0, 0, 0)) # the date portion is irrelevant
...
DRAWER_WIDTH = 400
DRAWER_OVER_WIDTH = IS_IOS and DRAWER_WIDTH or DRAWER_WIDTH * .9
...
DEFAULT_SCENE_SCALE = .33 # so default person size is largest size
DEEMPHASIZED_OPACITY = .1
BUTTON_SIZE = 36
...
OPENGL_SAMPLES = 16
SNAP_THRESHOLD_PERCENT = .25 # percent of dragging person height
SNAP_PEN = QPen(QColor(0, 0, 255, 100), .5)
...
EVENT_PROPS_HELP_TEXT = LONG_TEXT("""Events are occurrences in time.
...
EMOTION_PROPS_HELP_TEXT = LONG_TEXT("""Relationships describe
...
def scaleForPersonSize(size):
    if size > NORMAL_PERSON_SIZE:
        return 1.0 + .25 * (size - NORMAL_PERSON_SIZE)
    elif size == NORMAL_PERSON_SIZE:
        return .8
    elif size < NORMAL_PERSON_SIZE:
        return .4 ** abs(size - NORMAL_PERSON_SIZE)
...
def validatedDateTimeText(dateText, timeText=None):
    ...
    if timeText not in (None, '', BLANK_TIME_TEXT):
        ...
        if dt2:
            ...
            ret.setTime(QTime(dt2.hour, dt2.minute, dt2.second, int(dt2.microsecond / 1000)))
    return ret
...
def dateRangesOverlap(startA, endA, startB, endB):
    if (not startA and not endA) or (not startB and not endB):
        return True
    if startA is None: startA = QDateTime()
    if endA is None: endA = QDateTime()
    if startB is None: startB = QDateTime()
    if endB is None: endB = QDateTime()
...
def setButtonToolTip(button, action):
    text = '%s (%s)' %(action.toolTip(), action.shortcut().toString(QKeySequence.NativeText))
...
def qtHTTPReply2String(reply):
    ...
    message = '\n'.join([
        ...
        f"    RESPONSE HEADERS:",
        *[f"        {bytes(k).decode()}, {bytes(v).decode()}" for k, v in reply.rawHeaderPairs()]
    ])
    return message
...
def ____iCloudInitialized():
    ...
    DATA_PATH_LOCAL = os.path.join(QStandardPaths.standardLocations(QStandardPaths.DataLocation)[0], 'Documents')
    ...
    if not IS_BUNDLE and not IS_IOS and prefs.value('iCloudDataPath'): # dev only
        ...
        dir = QDir(DATA_PATH)
        if not dir.exists():
            Debug('util.init(): creating dir for QStandardPaths.DataLocation:', DATA_PATH)
