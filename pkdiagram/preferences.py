...
class Preferences(QDialog):
    ...
    def init(self, prefs):
        ...
        self.ui.reopenFileBox.setChecked(self.prefs.value('reopenLastFile', defaultValue=True, type=bool))
        self.ui.enablePinchZoomBox.setChecked(util.ENABLE_PINCH_PAN_ZOOM)
        util.ENABLE_WHEEL_PAN = self.prefs.value('enableWheelPan', defaultValue=True, type=bool)
        self.ui.enableWheelPanBox.setChecked(util.ENABLE_WHEEL_PAN)
        #
        checkForUpdatesAutomatically = self.prefs.value('checkForUpdatesAutomatically', defaultValue=True, type=bool)
        ...
        #
        darkLightMode = self.prefs.value('darkLightMode', defaultValue=util.PREFS_UI_HONOR_SYSTEM_DARKLIGHT_MODE)
        ...
        enableAppUsageAnalytics = self.prefs.value('enableAppUsageAnalytics', defaultValue=True, type=bool)
    ...
    def onDarkLightMode(self, on):
        if self.ui.darkLightSystemBox.isChecked():
            self.prefs.setValue('darkLightMode', util.PREFS_UI_HONOR_SYSTEM_DARKLIGHT_MODE)
    ...
    def oniCloudDrive(self, on):
        if on and on != util.usingOrSimulatingiCloud():
            ...
            stillOn = util.usingOrSimulatingiCloud() # may be canceled in C code; need to check
    ...
    def onSetDocsPath(self):
        localDocsPath = self.prefs.value('localDocsPath', type=str)
        fpath = QFileDialog.getExistingDirectory(QApplication.activeWindow(), 'Choose folder',
                                                 localDocsPath)
        if not fpath:
            return
