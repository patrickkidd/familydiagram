from .pyqt import (
    Qt, QScrollArea, QWidget, QPainter, QSize, QColor, QPoint, QApplication,
    QImage, QPixmap, QLabel, QTransform, QSlider, QHBoxLayout, QAction, QMenu, QWidgetAction
)
...
class ToolBar(QScrollArea):
    ...
    def addItems(self, items):
        for index, (name, attrs) in enumerate(items):
            ...
            if isinstance(attrs, dict):
                ...
                ignoreToggle = attrs.get('ignoreToggle', False) # don't draw `down` when toggled
                ...
                if slot:
                    ...
                    def makeOnActionChanged(action, button, ignoreToggle):
                        def onActionChanged():
                            ...
                            if not ignoreToggle and action.isChecked() != button.isChecked():
                            ...
                            def onButtonToggled(on=None):
                                ...
                                if on != action.isChecked():
                                    ...
                                    if action.actionGroup():
                                        action.actionGroup().triggered.emit(action) # qt bug fix?
                            return onButtonToggled
                        onButtonToggled = makeOnButtonToggled(button, action, ignoreToggle)
                        if ignoreToggle:
    ...
    def onResponsive(self, size):
        ...
        for index, (name, attrs) in enumerate(self.items):
            ...
            if w not in self.shouldShow:
                continue
            if self.position == 'north':
                inBounds = bool(w.x() + w.width() < size.width() - self.RESPONSIVE_MARGIN * 2)
            elif self.position in ('west', 'east'):
                inBounds = bool(w.y() + w.height() < size.height() - self.RESPONSIVE_MARGIN * 2)
        ...
        for label in self.helpTipLabels:
            viewPos = label._button.mapTo(self.parent(), label._button.pos()) # helpOverlay coordinates are the same as view coordinates
            ...
            if self.position == 'west':
                viewPosHacked = QPoint(int(self.width() + util.MARGIN_X * 2), int(firstButtonPos.y() + yOff))
            elif self.position == 'north':
                viewPosHacked = QPoint(int(firstButtonPos.x() + xOff + 15), int(self.height() + util.MARGIN_Y))
            elif self.position == 'east':
                viewPosHacked = QPoint(int(self.parent().width() - self.width() - util.MARGIN_X * 2 - label.width()), int(firstButtonPos.y() + yOff))
    ...
    def adjust(self):
        ...
        if self.position == 'north':
            self.widget().resize(
                self.cur,
                util.BUTTON_SIZE + self.MARGIN_Y * 2
            )
        elif self.position in ('west', 'east'):
            self.widget().resize(
                util.BUTTON_SIZE + self.MARGIN_X * 2,
                self.cur + self.MARGIN_Y
            )
...
class ItemToolBar(ToolBar):
    ...
    def __init__(self, parent, ui):
        ...
        self.pencilSlider.setFixedWidth(80) # avoid jitter from label changing width with value.
