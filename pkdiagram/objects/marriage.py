import logging
from ..pyqt import QGraphicsItem, QPen, QPainterPath, QPointF, QDateTime, pyqtSignal, Qt, QFont, QRectF
from .. import util
...
class SeparationIndicator(PathItem):
    ...
    def updateGeometry(self):
        ...
        lineRise = personRect.height() * .4
        lineRun = personRect.width() * .2 * custodyDirection
        y = personRect.height() * .15
...
class Marriage(PathItem):
    ...
    def personConnectionPoint(person):
        ...
        r = QPointF(rect.bottomLeft().x() + (rect.bottomRight().x() - rect.bottomLeft().x()) / 2, rect.bottomLeft().y()) # - stroke)
    ...
    def __init__(self, personA=None, personB=None, **kwargs):
        ...
        self.children = [] # used for geometry changes and to notify children when deleting marriage.
        # self.centerItem = QGraphicsEllipseItem(QRectF(-3, -3, 6, 6), self)
        self.detailsText = ItemDetails(self, growUp=True)
        self.detailsText.setPos(QPointF(30, -self.detailsText.boundingRect().bottomLeft().y()))
        font = QFont(util.DETAILS_FONT)
        font.setPointSize(round(font.pointSize() * .9))
    ...
    def separationStatusFor(self, dateTime):
        ...
        divorcedEvents = []
        for e in self._events:
            if e.uniqueId() == 'separated' and e.dateTime() and e.dateTime() <= dateTime:
                separatedEvents.append(e)
            elif e.uniqueId() == 'divorced' and e.dateTime() and e.dateTime() <= dateTime:
    ...
    def _onAddChild(self, c):
        if not c in self.children:
            ...
            self.children = sorted(self.children, key=lambda x: x.id is not None and x.id or 0)
    ...
    def updateGeometry(self):
        ...
        newScenePos = QPointF(newPathSceneRect.bottomLeft().x() + newPathSceneRect.width() / 2, newPathSceneRect.bottomLeft().y())
    ...
    def updateDetails(self):
        ...
        for event in events:
            ...
            if not event.dateTime() or not currentDateTime or event.dateTime() > currentDateTime:
                continue
            if uniqueId == 'bonded' and event.dateTime():
                ...
                lines.append('%s %s' % (util.dateString(event.dateTime()), event.description()))
    ...
    def shouldShowFor(self, dateTime, tags=[], layers=[]):
        ...
        for child in self.children:
            if child.shouldShowFor(dateTime, tags=tags, layers=layers):
                return True
        # 5
        anyBondedMarriedDates = [e.dateTime() for e in self._events if e.uniqueId() in ('bonded', 'married')]
        priorBondedMarriedDates = [d for d in anyBondedMarriedDates if d is not None and d <= dateTime]
