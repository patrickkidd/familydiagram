...
class Jig:
    ...
    def __init__(
        ...
        if pointA is None and personA:
            pointA = personA.mapToScene(
                personA.boundingRect().center()
            )  # TODO: use personA.pos()?
        if circA is None and personA:  # circumference from original person
            personWidth = (
                personA.mapToScene(personA.boundingRect()).boundingRect().width()
            )
            if personA.gender() == "male":
                circA = personWidth - personWidth * 0.25
            else:
                circA = personWidth - personWidth * 0.2
        if pointB is None and personB:
            pointB = personB.mapToScene(personB.boundingRect().center())
            if circB is None:
                personWidth = (
                    personB.mapToScene(personB.boundingRect()).boundingRect().width()
                )
                if personA.gender() == "male":
                    circB = personWidth - personWidth * 0.25
                else:
                    circB = personWidth - personWidth * 0.2
        if circB is None:
            circB = circA
...
class FannedBox(QGraphicsObject):
    ...
    def updateGeometry(self):
        ...
        for emotion, entry in self.entries.items():
            rect = rect.united(
                emotion.mapToScene(emotion.boundingRect()).boundingRect()
            )
    ...
    def updateFannedState(self):
        ...
        for emotion, alt in self._steps():
            ...
            entries[emotion].update(
                {"alt": alt, "endPosDelta": self.endPosDeltaFor(emotion)}
            )
...
def pathFor_Conflict(
    personA, personB=None, pointB=None, withClip=False, intensity=1, **kwargs
):
    path = QPainterPath()
    size = util.sizeForPeople(personA, personB)
    scale = util.scaleForPersonSize(size)
...
def pathFor_Distance(personA, personB=None, pointB=None, intensity=1, **kwargs):
    ...
    path = QPainterPath()
    gap = jig.width * 1.5
    rightMidStop = CUtil.pointOnRay(
        jig.aP, jig.bP, QLineF(jig.aP, jig.bP).length() * 0.5 - gap
    )
    if intensity == 1:
    ...
    path.moveTo(cp1)
    path.lineTo(cp2)
    leftMidStop = CUtil.pointOnRay(
        jig.bP, jig.aP, QLineF(jig.aP, jig.bP).length() * 0.5 - gap
    )
    if intensity == 1:
...
def pathFor_Cutoff(personA=None, personB=None, pointB=None, intensity=1, **kwargs):
    ...
    if personA:
        if personA.gender() == "male":
            rect.moveCenter(rect.center() + QPointF(0, -w * 0.5))  # slight nudge up
        if personA.primary():
            rect.moveCenter(
                rect.center() + QPointF(0, -w * 0.5)
            )  # another slight nudge up
...
def pathFor_Toward(personA, personB=None, pointB=None, intensity=1, **kwargs):
    ...
    # Mover (Person A)
    rightArrowPoint = CUtil.pointOnRay(
        jig.aP, jig.bP, QLineF(jig.aP, jig.bP).length() * 0.5 - gap
    )
    # Mover arrow line(s)
    if intensity == 1:
        rightArrowStop = CUtil.pointOnRay(
            jig.aP, jig.bP, QLineF(jig.aP, jig.bP).length() * 0.5 - gap
        )
        ...
        rightArrowStop = CUtil.pointOnRay(
            jig.aP, jig.bP, QLineF(jig.aP, jig.bP).length() * 0.5 - gap * 2.2
        )
        ...
        path.lineTo(p4a)
    # Mover arrow head
    ap1 = CUtil.pointOnRay(
        jig.p1, jig.p2, QLineF(jig.aP, jig.bP).length() * 0.5 - gap * 2
    )
    ap2 = CUtil.pointOnRay(
        jig.p4, jig.p3, QLineF(jig.aP, jig.bP).length() * 0.5 - gap * 2
    )
    ...
    # Recipient (Person B)
    leftArrowPoint = CUtil.pointOnRay(
        jig.bP, jig.aP, QLineF(jig.bP, jig.aP).length() * 0.5 - gap
    )
    leftArrowStop = CUtil.pointOnRay(
        jig.bP, jig.aP, QLineF(jig.bP, jig.aP).length() * 0.5 - gap * 2
    )
    # Recipient line
...
def pathFor_DefinedSelf(personA, personB=None, pointB=None, intensity=1, **kwargs):
    ...
    if intensity == 1:
        ...
        leftArrowStop = CUtil.pointOnRay(
            jig.bP, jig.aP, QLineF(jig.bP, jig.aP).length() * 0.5 - gap * 2.2
        )
...
def pathFor_Inside(personA, personB=None, pointB=None, intensity=1, **kwargs):
    ...
    rightArrowStop = CUtil.pointOnRay(
        jig.aP, jig.bP, QLineF(jig.aP, jig.bP).length() * 0.5 - gap * 2
    )
    # right arrow line(s)
    if intensity == 1:
        path.moveTo(rightArrowStop)
    ...
    # left arrow head
    ap1 = CUtil.pointOnRay(
        jig.p2, jig.p1, QLineF(jig.bP, jig.aP).length() * 0.5 - gap * 2
    )
    ap2 = CUtil.pointOnRay(
        jig.p3, jig.p4, QLineF(jig.bP, jig.aP).length() * 0.5 - gap * 2
    )
    path.moveTo(leftArrowPoint)
    path.lineTo(ap1)
    path.moveTo(leftArrowPoint)
...
def pathFor_Reciprocity(
    personA, personB=None, pointB=None, intensity=1, hoverPerson=None, **kwargs
):
    path = QPainterPath()
    ...
    armH = 15 * scale
    armW = 15 * scale
    h = personARect.height() * 0.5  # arrow height
    ...
    if personB is not None:
        ...
        armH = 15 * scale
        armW = 15 * scale
        h = personBRect.height() * 0.5  # arrow height
        ...
        if peers:
            ...
            for peer in peers:
                if peer.fannedBox:
                    foundInPeer = (
                        peer.fannedBox
                    )  # defered to after fan-in animation in .addEmotion()
                    break
