...
class QtInspector:
    ...
    def hover(self, objectName: str, pos: Optional[tuple] = None) -> Dict[str, Any]:
        widget = self._findWidget(objectName)
        if widget is not None:
    ...
    def scroll(
        self, objectName: str, direction: str, amount: int = 100
    ) -> Dict[str, Any]:
        """Scroll by simulating a touch swipe drag. direction: up/down/left/right."""
        item = self._findQmlItem(objectName)
        widget = self._findWidget(objectName) if item is None else None
        ...
        startPos, endPos = dirMap[direction]
        return self.drag(objectName, startPos, endPos)
    ...
    def getText(self, objectName: str) -> Dict[str, Any]:
        widget = self._findWidget(objectName)
        if widget is not None:
            return {"success": True, "text": self._getTextProperty(widget)}

        item = self._findQmlItem(objectName)
        if item is not None:
            for prop in ["text", "displayText", "plainText", "title", "label"]:
                val = item.property(prop)
                if isinstance(val, str) and val:
                    return {"success": True, "text": val}
            contentItem = item.property("contentItem")
            if contentItem:
                for prop in ["text", "displayText"]:
                    val = contentItem.property(prop)
                    if isinstance(val, str) and val:
                        return {"success": True, "text": val}
    ...
    def getBounds(self, objectName: str) -> Dict[str, Any]:
        widget = self._findWidget(objectName)
        if widget is not None:
            globalPos = widget.mapToGlobal(QPoint(0, 0))
            rect = widget.rect()
    ...
    def resizeWindow(self, width: int, height: int) -> Dict[str, Any]:
        for window in self._app.topLevelWindows():
            if isinstance(window, QQuickWindow) and window.isVisible():
