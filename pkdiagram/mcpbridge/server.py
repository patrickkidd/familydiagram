...
class TestBridgeServer(QObject):
    ...
    def _setupHandlers(self):
        ...
        self._handlers = {
            ...
            # iOS-compatible interaction primitives
            "hover": self._handleHover,
            "scroll": self._handleScroll,
            "long_press": self._handleLongPress,
            "get_text": self._handleGetText,
            "get_bounds": self._handleGetBounds,
            "resize_window": self._handleResizeWindow,
            # Scene items
    ...
    def _handleHover(self, command: Dict) -> Dict:
        objectName = command.get("objectName")
        if not objectName:
            return {"success": False, "error": "Missing 'objectName'"}
        pos = command.get("pos")
        if pos:
            pos = tuple(pos)
        return self._inspector.hover(objectName, pos)

    def _handleScroll(self, command: Dict) -> Dict:
        objectName = command.get("objectName")
        if not objectName:
            return {"success": False, "error": "Missing 'objectName'"}
        direction = command.get("direction", "up")
        amount = command.get("amount", 100)
        return self._inspector.scroll(objectName, direction, amount)

    def _handleLongPress(self, command: Dict) -> Dict:
        objectName = command.get("objectName")
        if not objectName:
            return {"success": False, "error": "Missing 'objectName'"}
        durationMs = command.get("durationMs", 500)
        pos = command.get("pos")
        if pos:
            pos = tuple(pos)
        return self._inspector.longPress(objectName, durationMs, pos)

    def _handleGetText(self, command: Dict) -> Dict:
        objectName = command.get("objectName")
        if not objectName:
            return {"success": False, "error": "Missing 'objectName'"}
        return self._inspector.getText(objectName)

    def _handleGetBounds(self, command: Dict) -> Dict:
        objectName = command.get("objectName")
        if not objectName:
            return {"success": False, "error": "Missing 'objectName'"}
        return self._inspector.getBounds(objectName)

    def _handleResizeWindow(self, command: Dict) -> Dict:
        width = command.get("width")
        height = command.get("height")
        if not width or not height:
            return {"success": False, "error": "Missing 'width' or 'height'"}
        return self._inspector.resizeWindow(int(width), int(height))
