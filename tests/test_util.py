...
def test_date_overlap():
    ...
    # inside
    assert util.dateRangesOverlap(date(2000), date(2003),
                            date(2001), date(2002))
    # before + inside
    assert util.dateRangesOverlap(date(2000), date(2002),
                            date(1999), date(2001))
    # inside + after
    assert util.dateRangesOverlap(date(2000), date(2002),
                            date(2001), date(2004))
    # before
    assert not util.dateRangesOverlap(date(2000), date(2001),
                                date(1998), date(1999))
    # after
    assert not util.dateRangesOverlap(date(2000), date(2001),
                                date(2002), date(2003))
...
def test_QRect_contains():
    CUtil = util.CUtil
    assert CUtil.QRect_contains_QRect(QRect(0, 0, 10, 10),
                                      QRect(1, 1, 9, 9))
    assert not CUtil.QRect_contains_QRect(QRect(0, 0, 10, 10),
                                          QRect(-1, -1, 9, 9))
    assert not CUtil.QRect_contains_QRect(QRect(1, 1, 9, 9),
                                          QRect(0, 0, 10, 10))
