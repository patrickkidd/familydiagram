...
class TestRemoveChildOf:
    ...
    def test_remove_child_relationship(self, scene):
        ...
        assert len(scene.query(types=ChildOf)) == 1
    ...
    def test_remove_marriage_deletes_children(self, scene):
        ...
        assert len(scene.query(types=ChildOf)) == 2
        assert child1.childOf == childOf1
        assert child2.childOf == childOf2
...
class TestRemoveBirthPartners:
    ...
    def test_remove_one_twin_reattach_to_existing_multiplebirth(self, scene):
        ...
        marriage = scene.addItem(Marriage(parent1, parent2))
        childOf1 = scene.addItem(ChildOf(twin1, marriage))
        childOf2 = scene.addItem(ChildOf(twin2, marriage))
        childOf3 = scene.addItem(ChildOf(twin3, marriage))
        multipleBirth = scene.addItem(MultipleBirth(marriage))
        childOf1.setMultipleBirth(multipleBirth)
        childOf2.setMultipleBirth(multipleBirth)
        ...
        assert twin3.childOf.multipleBirth == multipleBirth
...
class TestComplexChildScenarios:
    ...
    def test_sequential_child_operations(self, scene):
        ...
        scene.removeItem(childOf1, undo=True)
        assert len(scene.query(types=ChildOf)) == 1

        scene.removeItem(childOf2, undo=True)
        assert len(scene.query(types=ChildOf)) == 0
    ...
    def test_remove_multiple_children_at_once(self, scene):
        ...
        childOf1 = scene.addItem(ChildOf(child1, marriage))
        childOf2 = scene.addItem(ChildOf(child2, marriage))
        childOf3 = scene.addItem(ChildOf(child3, marriage))
