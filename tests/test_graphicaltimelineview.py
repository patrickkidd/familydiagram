...
from pkdiagram import util, GraphicalTimelineView, GraphicalTimelineCanvas, Scene, Event, Person
...
def create_gtv(qtbot):
    ...
    def _create_gtv():
        ...
        scene = Scene()
        folks = (
            Person(birthDateTime=QDateTime(QDate(2000 + i, 1, 1)))
            for i in range(5)
        )
...
def test_scene_setCurrentDate_from_graphical_timeline_click(qtbot, create_gtv):
    ...
    pos = QPoint(int(gtv.timeline.canvas.width() / 3), int(gtv.height() / 3)) # sort of 1/3 upper left
    date = gtv.timeline.canvas._dateTimeForPoint(pos).date() # lose time precision for easier equality
