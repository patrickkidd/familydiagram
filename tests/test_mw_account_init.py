...
@pytest.mark.parametrize('is_server_down', [True, False])
def test_dont_show_account_with_session_pro_license(test_license, test_activation, create_ac_mw, server_down, is_server_down):
    with server_down(is_server_down):
        ac, mw = create_ac_mw()
        assert mw.session.activeFeatures() == [vedana.LICENSE_PROFESSIONAL] # vedana.licenses_features([test_license.as_dict(include='policy')])
        assert mw.accountDialog.isShown() == False


@pytest.mark.parametrize('is_server_down', [True, False])
def test_logout(test_license, test_activation, qtbot, create_ac_mw, server_down, is_server_down):
    with server_down(is_server_down):
        ac, mw = create_ac_mw()
...
def test_dont_logout_when_show_account_with_session_pro_license(test_license, test_activation, create_ac_mw, server_down):
    with server_down():
        ac, mw = create_ac_mw()
        pre_features = vedana.licenses_features([test_license.as_dict(include='policy')])
        mw.showAccount()
