...
from pkdiagram import util, Scene, Person, Event, QmlWidgetHelper, SceneModel, Session, CaseProperties, AccessRightsModel
...
def test_add_access_right_as_client(test_user, test_user_2, test_client_activation, create_cp):
...
def test_add_only_one_access_right_as_client(test_user, test_user_2, test_client_activation, qtbot, create_cp):
    test_user_3 = User(
    ...
    qtbot.clickOkAfter(
        lambda: cp.keyClicks('addAccessRightBox', test_user_3.username, returnToFinish=True),
        contains=AccessRightsModel.S_CLIENT_ONLY_ALLOWED_ONE_RIGHT
    )
    diagram = Diagram.query.get(test_user.free_diagram.id)
    assert len(diagram.access_rights) == 1, "Client should not be able to add more than one access right."


def test_add_one_access_right_for_free_as_client(test_user, test_user_2, test_client_activation, qtbot, create_cp):
    cp = create_cp(loadFreeDiagram=True)
    db.session.add(test_user)
    data = pickle.loads(test_user.free_diagram.data)
    cp.sceneModel.scene.read(data)
    ...
    qtbot.clickOkAfter(lambda: cp.keyClicks('addAccessRightBox', test_user_2.username, returnToFinish=True), text='already exists.')
    assert len(diagram.access_rights) == 1


def test_edit_access_right(test_user, test_user_2, test_client_activation, create_cp):
    test_user.free_diagram.grant_access(test_user_2, vedana.ACCESS_READ_WRITE, _commit=True)
...
def test_serverBox_enabled_with_client_license(tmp_path, test_session, test_client_activation, create_cp, is_on_server):
    if is_on_server:
        cp = create_cp(loadFreeDiagram=True)
    else:
        cp = create_cp()
