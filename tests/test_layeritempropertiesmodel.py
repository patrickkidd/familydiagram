...
def layerItemStuff():
    ...
    addLayer()
    callout = Callout(
        layers=[l.id for i, l in enumerate(scene.layers()) if i in (1, 3)]
    )
...
def test_set_different_item_layer_active_but_enforce_min_one_layer(layerItemStuff):
    ...
    assert (
        model.data(model.index(1, 0), model.ActiveRole) == Qt.PartiallyChecked
    )  # [True, False]
    assert (
        model.data(model.index(2, 0), model.ActiveRole) == Qt.Unchecked
    )  # [False, False]
    assert (
        model.data(model.index(3, 0), model.ActiveRole) == Qt.Unchecked
    )  # [False, False]
    ...
    assert dataChanged.callArgs[0][2][0] == model.ActiveRole
    assert (
        model.data(model.index(0, 0), model.ActiveRole) == Qt.PartiallyChecked
    )  # [False, True] -- callout2 remains
    assert (
        model.data(model.index(1, 0), model.ActiveRole) == Qt.PartiallyChecked
    )  # [True, False]
    assert (
...
def test_set_different_item_layer_active(layerItemStuff):
    ...
    callout2 = Callout(
        layers=[l.id for i, l in enumerate(scene.layers()) if i in (0, 3)]
    )
    scene.addItem(callout2)
    model.items = [callout, callout2]
    dataChanged = util.Condition()
    ...
    assert dataChanged.callArgs[1][2][0] == model.ActiveRole
    assert (
        model.data(model.index(0, 0), model.ActiveRole) == Qt.Unchecked
    )  # [False, False]
    assert (
        model.data(model.index(1, 0), model.ActiveRole) == Qt.PartiallyChecked
    )  # [True, False]
