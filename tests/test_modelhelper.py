...
class Model(QObject, ModelHelper):
    ...
    PROPERTIES = objects.Item.adjustedClassProperties(MyItem, [
        { 'attr': 'newEntry', 'type': int, 'default': 678 },
        { 'attr': 'newEntryNoType' }
    ])
...
def test_default():
    ...
    itemWithValues = MyItem(noDefaultNoType='asd',
                            noDefaultWithType=777,
                            defaultWithType=1333)
...
def test_convertTo():
    ...
    assert model.someBool == Qt.PartiallyChecked # because &.same() returns None, which is converted to partial check
