...
def test_init_deinit(timelineScene, model):
    ...
    assert model.index(0, col).data(Qt.DisplayRole) == \
                     util.dateString(p1.birthDateTime())
    assert model.index(1, col).data(Qt.DisplayRole) == \
                     util.dateString(p2.birthDateTime())
    # unsure
    ...
    assert model.index(0, col).data(Qt.DisplayRole) == \
                     util.dateString(QDateTime.currentDateTime())
    assert model.index(1, col).data(Qt.DisplayRole) == \
                     util.dateString(QDateTime.currentDateTime())
    # nowEvent is last
...
def test_shouldHide():
    ...
    assert model._shouldHide(Event(dateTime=util.Date(2000, 1, 1), tags=['here'])) == False

    scene.searchModel.tags = []
    scene.searchModel.description = 'there'
    assert model._shouldHide(Event(dateTime=util.Date(2000, 1, 1))) == True
    assert model._shouldHide(Event(dateTime=util.Date(2000, 1, 1), tags=['here'])) == True
    assert model._shouldHide(Event(dateTime=util.Date(2000, 1, 1), description=['therefore'])) == False
...
def test_init_multiple_people():
    ...
    eventA = Event(parent=personA, description='PersonA something', dateTime=util.Date(2002, 1, 1))
    eventB = Event(parent=personB, description='PersonB something', dateTime=util.Date(2003, 1, 1))
    # add two emotions where only one person's end should should in the timeline
    emotionA = Emotion(kind=util.ITEM_DISTANCE, personA=personA, personB=personC,
                       startDateTime=util.Date(2004, 1, 1), endDateTime=util.Date(2005, 1, 1)) 
    emotionB = Emotion(kind=util.ITEM_CONFLICT, personA=personB, personB=personC,
                       startDateTime=util.Date(2006, 1, 1), endDateTime=util.Date(2007, 1, 1)) 
    # add one emotion where both ends should be shown but without duplicates.
    emotionC = Emotion(kind=util.ITEM_FUSION, personA=personA, personB=personB,
                       startDateTime=util.Date(2008, 1, 1), endDateTime=util.Date(2009, 1, 1)) 
    ...
    assert model.data(model.index(0, 0), model.DateTimeRole) == personA.birthEvent.dateTime()
    assert model.data(model.index(1, 0), model.DateTimeRole) == personB.birthEvent.dateTime()
...
def test_include_marriage_events():
    ...
    separated = Event(parent=marriage, uniqueId='separated', dateTime=util.Date(2001, 1, 1))
...
def test_add_item(timelineScene, model):
    ...
    # fusion dates
    col = model.columnIndex(model.DATETIME)
    assert model.index(2, col).data(Qt.DisplayRole) == util.dateString(fusion.startDateTime())
    assert model.index(3, col).data(Qt.DisplayRole) == util.dateString(fusion.endDateTime())
...
def test_delete_emotion_date(qtbot, timelineScene, model):
    ...
    selectionModel = QItemSelectionModel(model)
    selectionModel.select(model.index(3, 0), QItemSelectionModel.Select | QItemSelectionModel.Rows)
    qtbot.clickYesAfter(lambda: model.removeSelection(selectionModel))
    assert model.rowCount() == 4
    assert conflict.endEvent.dateTime() == None
    assert conflict.endEvent in timelineScene.events()

    selectionModel.select(model.index(2, 0), QItemSelectionModel.Select | QItemSelectionModel.Rows)
    qtbot.clickYesAfter(lambda: model.removeSelection(selectionModel))
    assert model.rowCount() == 3
...
@pytest.mark.skip(reason="Unsure how to consider Person.deceased from TimelineModel._ensureEvent().")
...
def test_set_emotion_date():
    ...
    assert model.data(model.index(0, iDate), model.DateTimeRole) == util.Date(1991, 1, 1)
    assert model.data(model.index(1, iDate), model.DateTimeRole) == util.Date(1992, 1, 1)
...
def test_showAliases_signals():
    ...
    e1 = Event(parent=patrick, dateTime=util.Date(1900, 1, 1), description='Bob came home')
    e2 = Event(parent=patrick, dateTime=util.Date(1900, 1, 2), description="robby came home, took Robby's place")
    e3 = Event(parent=bob, dateTime=util.Date(1900, 1, 3), description='Patrick came home with bob')
    distance = Emotion(kind=util.ITEM_DISTANCE,
                       startDateTime=util.Date(1900, 1, 4), endDateTime=util.Date(1900, 1, 5),
                       personA=patrick, personB=bob)
    marriage = Marriage(personA=patrick, personB=bob)
    marriedEvent = Event(parent=marriage, uniqueId='married', dateTime=util.Date(1900, 1, 5))
