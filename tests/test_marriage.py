...
def simpleMarriage(qmlScene, request):
    personA, personB = Person(), Person()
    marriage = Marriage(personA=personA, personB=personB)
    Event(parent=marriage, uniqueId='bonded', dateTime=util.Date(1900, 1, 1))
    Event(parent=marriage, uniqueId='married', dateTime=util.Date(1910, 1, 1))
    Event(parent=marriage, uniqueId='separated', dateTime=util.Date(1920, 1, 1))
    Event(parent=marriage, uniqueId='moved', dateTime=util.Date(1925, 1, 1), location='Washington, DC')
    Event(parent=marriage, uniqueId='married', dateTime=util.Date(1930, 1, 1))
    Event(parent=marriage, uniqueId='separated', dateTime=util.Date(1940, 1, 1))
    Event(parent=marriage, uniqueId='divorced', dateTime=util.Date(1950, 1, 1))
    qmlScene.addItems(personA, personB, marriage)
    request.addfinalizer(lambda: qmlScene.deinit())
    return marriage
...
def test_shouldShowFor_one_parent_hidden(monkeypatch, noEvents):
    marriage = noEvents
    monkeypatch.setattr(marriage.people[0], 'shouldShowFor', lambda x, tags=[], layers=[]: False)
    monkeypatch.setattr(marriage.people[1], 'shouldShowFor', lambda x, tags=[], layers=[]: True)
...
def test_shouldShowFor_both_parents_hidden(monkeypatch, noEvents):
    marriage = noEvents
    monkeypatch.setattr(marriage.people[0], 'shouldShowFor', lambda x, tags=[], layers=[]: False)
    monkeypatch.setattr(marriage.people[1], 'shouldShowFor', lambda x, tags=[], layers=[]: False)
    assert marriage.shouldShowFor(QDateTime.currentDateTime()) == False


def test_shouldShowFor_no_parents_hidden(monkeypatch, noEvents):
    marriage = noEvents
    monkeypatch.setattr(marriage.people[0], 'shouldShowFor', lambda x, tags=[], layers=[]: True)
    monkeypatch.setattr(marriage.people[1], 'shouldShowFor', lambda x, tags=[], layers=[]: True)
...
def test_shouldShowFor_both_parents_n_all_children(monkeypatch, noEvents2Children):
    ...
    monkeypatch.setattr(marriage.people[0], 'shouldShowFor', lambda x, tags=[], layers=[]: True)
    monkeypatch.setattr(marriage.people[1], 'shouldShowFor', lambda x, tags=[], layers=[]: True)
    monkeypatch.setattr(marriage.children[0], 'shouldShowFor', lambda x, tags=[], layers=[]: True)
    monkeypatch.setattr(marriage.children[1], 'shouldShowFor', lambda x, tags=[], layers=[]: True)
    assert marriage.shouldShowFor(QDateTime.currentDateTime()) == True
...
def test_shouldShowFor_both_parents_n_one_child(monkeypatch, noEvents2Children):
    ...
    monkeypatch.setattr(marriage.people[0], 'shouldShowFor', lambda x, tags=[], layers=[]: True)
    monkeypatch.setattr(marriage.people[1], 'shouldShowFor', lambda x, tags=[], layers=[]: True)
    monkeypatch.setattr(marriage.children[0], 'shouldShowFor', lambda x, tags=[], layers=[]: False)
    monkeypatch.setattr(marriage.children[1], 'shouldShowFor', lambda x, tags=[], layers=[]: True)
    assert marriage.shouldShowFor(QDateTime.currentDateTime()) == True
...
def test_shouldShowFor_both_parents_n_no_children(monkeypatch, noEvents2Children):
    ...
    monkeypatch.setattr(marriage.people[1], 'shouldShowFor', lambda x, tags=[], layers=[]: True)
    monkeypatch.setattr(marriage.children[0], 'shouldShowFor', lambda x, tags=[], layers=[]: False)
    monkeypatch.setattr(marriage.children[1], 'shouldShowFor', lambda x, tags=[], layers=[]: False)
    assert marriage.shouldShowFor(QDateTime.currentDateTime()) == True
...
@pytest.mark.parametrize('uniqueId', ['bonded', 'married'])
def test_shouldShowFor_first_bonded_event_prior_to_first_child(monkeypatch, noEvents2Children, uniqueId):
    marriage = noEvents2Children
    Event(parent=marriage, uniqueId=uniqueId, dateTime=util.Date(1990, 1, 1)) # prior to child births
    monkeypatch.setattr(marriage.people[0], 'shouldShowFor', lambda x, tags=[], layers=[]: True)
    monkeypatch.setattr(marriage.people[1], 'shouldShowFor', lambda x, tags=[], layers=[]: True)
    ...
    assert marriage.shouldShowFor(util.Date(1995, 1, 1)) == True # between event and first birth


# Children incorrectly added prior to bonded/married events)
@pytest.mark.parametrize('uniqueId', ['bonded', 'married'])
def test_shouldShowFor_first_bonded_married_event_after_first_child(noEvents2Children, uniqueId):
    marriage = noEvents2Children
    marriage.children[0].setBirthDateTime(util.Date(1990, 1, 1))
    marriage.children[1].setBirthDateTime(util.Date(1990, 1, 1))
    Event(parent=marriage, uniqueId=uniqueId, dateTime=util.Date(2000, 1, 1)) # prior to child births
    assert marriage.shouldShowFor(util.Date(1980, 1, 1)) == False # prior to first child
    assert marriage.shouldShowFor(util.Date(1995, 1, 1)) == True # between first child and event
...
def test_penStyleFor_bonded_prior_to_first_child(noEvents2Children):
    ...
    Event(parent=marriage, uniqueId='bonded', dateTime=util.Date(1990, 1, 1)) # prior to child births
    marriage.children[0].setBirthDateTime(util.Date(2000, 1, 1))
    marriage.children[1].setBirthDateTime(util.Date(2010, 1, 1))
    assert marriage.penStyleFor(util.Date(1980, 1, 1)) == Qt.DashLine # prior to event
    assert marriage.penStyleFor(util.Date(1995, 1, 1)) == Qt.DashLine # between event and first birth
    assert marriage.penStyleFor(util.Date(2005, 1, 1)) == Qt.DashLine # after first birth


def test_penStyleFor_married_prior_to_first_child(noEvents2Children):
    marriage = noEvents2Children
    marriage.setMarried(False)
    Event(parent=marriage, uniqueId='married', dateTime=util.Date(1990, 1, 1)) # prior to child births
    marriage.children[0].setBirthDateTime(util.Date(2000, 1, 1))
    marriage.children[1].setBirthDateTime(util.Date(2010, 1, 1))
    assert marriage.penStyleFor(util.Date(1980, 1, 1)) == Qt.DashLine # prior to event
    assert marriage.penStyleFor(util.Date(1995, 1, 1)) == Qt.SolidLine # between event and first birth
    assert marriage.penStyleFor(util.Date(2005, 1, 1)) == Qt.SolidLine # after first birth
...
def test_penStyleFor_bonded_after_first_child(noEvents2Children):
    ...
    marriage.setMarried(False)
    marriage.children[0].setBirthDateTime(util.Date(1990, 1, 1))
    Event(parent=marriage, uniqueId='bonded', dateTime=util.Date(2000, 1, 1)) # prior to child births
    assert marriage.penStyleFor(util.Date(1980, 1, 1)) == Qt.DashLine # prior to first child
    assert marriage.penStyleFor(util.Date(1995, 1, 1)) == Qt.DashLine # between first child and event
...
def test_penStyleFor_married_after_first_child(noEvents2Children):
    ...
    marriage.setMarried(False)
    marriage.children[0].setBirthDateTime(util.Date(1990, 1, 1))
    Event(parent=marriage, uniqueId='married', dateTime=util.Date(2000, 1, 1)) # prior to child births
    assert marriage.penStyleFor(util.Date(1980, 1, 1)) == Qt.DashLine # prior to first child
    assert marriage.penStyleFor(util.Date(1995, 1, 1)) == Qt.DashLine # between first child and event
...
def test_penStyleFor_married_no_married_events(noEvents):
    ...
    assert marriage.penStyleFor(QDateTime.currentDateTime()) == Qt.SolidLine # prior to first child


def test_penStyleFor_married_w_married_events(noEvents):
    marriage = noEvents
    marriage.setMarried(True)
    Event(parent=marriage, uniqueId='married', dateTime=util.Date(2000, 1, 1))
    Event(parent=marriage, uniqueId='married', dateTime=util.Date(2010, 1, 1))
    assert marriage.penStyleFor(QDateTime.currentDateTime()) == Qt.SolidLine # prior to first child
...
def test_penStyleFor_married_w_married_events_and_bonded_prior(noEvents):
    marriage = noEvents
    marriage.setMarried(True)
    Event(parent=marriage, uniqueId='bonded', dateTime=util.Date(1990, 1, 1))
    Event(parent=marriage, uniqueId='married', dateTime=util.Date(2000, 1, 1))
    Event(parent=marriage, uniqueId='married', dateTime=util.Date(2010, 1, 1))
    assert marriage.penStyleFor(util.Date(1995, 1, 1)) == Qt.DashLine # after bonded, prior to marriage
    assert marriage.penStyleFor(util.Date(2005, 1, 1)) == Qt.SolidLine # after marriage
...
def test_penStyleFor_divorced_w_divorced_events(noEvents):
    ...
    marriage.setMarried(False)
    marriage.setDivorced(True)
    Event(parent=marriage, uniqueId='divorced', dateTime=util.Date(2000, 1, 1))
    assert marriage.penStyleFor(QDateTime.currentDateTime()) == Qt.SolidLine # prior to first child
...
def test_penStyleFor_married_w_bonded_event_prior_no_married_events(noEvents):
    ...
    marriage.setMarried(True)
    Event(parent=marriage, uniqueId='bonded', dateTime=util.Date(1990, 1, 1))
    assert marriage.penStyleFor(util.Date(2005, 1, 1)) == Qt.SolidLine # after bonded event
...
def test_separationStatusFor_no_div_sep_event_div_no_sep(noEvents):
    ...
    marriage.setSeparated(False)
    marriage.setDivorced(True)
    assert marriage.separationStatusFor(QDateTime.currentDateTime()) == 'divorced'
...
def test_separationStatusFor_sep_event_no_div_event_no_div_sep(noEvents):
    ...
    marriage.setSeparated(False)
    marriage.setDivorced(False)
    Event(parent=marriage, uniqueId='separated', dateTime=util.Date(2000, 1, 1))
    assert marriage.separationStatusFor(QDateTime.currentDateTime()) == 'separated'
...
def test_separationStatusFor_one_moved_event_no_div_sep(noEvents):
    ...
    marriage.setSeparated(False)
    marriage.setDivorced(False)
    Event(parent=marriage, uniqueId='moved', location='Somewhere', dateTime=util.Date(1990, 1, 1))
    assert marriage.separationStatusFor(QDateTime.currentDateTime()) == None
...
def test_separationStatusFor_one_moved_event_sep_and_div_events(noEvents):
    ...
    marriage.setSeparated(False)
    marriage.setDivorced(False)
    Event(parent=marriage, uniqueId='separated', location='Somewhere', dateTime=util.Date(1990, 1, 1))
    Event(parent=marriage, uniqueId='moved', dateTime=util.Date(2000, 1, 1))
    Event(parent=marriage, uniqueId='divorced', dateTime=util.Date(2010, 1, 1))
    assert marriage.separationStatusFor(util.Date(1985, 1, 1)) == None          # before separation
    assert marriage.separationStatusFor(util.Date(1995, 1, 1)) == 'separated'   # after separation
    assert marriage.separationStatusFor(util.Date(2005, 1, 1)) == 'separated'   # after move
    assert marriage.separationStatusFor(util.Date(2015, 1, 1)) == 'divorced'    # after divorce
...
def test_lines(simpleMarriage):
    somethingHappened = Event(parent=simpleMarriage, description='Something happened', dateTime=util.Date(1922, 1, 1))
...
def test_compat_marriage_events():
    import os.path
    with open(os.path.join(conftest.DATA_ROOT, 'v114a7_simplefamily.pickle'), 'rb') as f:
    ...
    events = marriage.events()
    assert len(events) == 5
    marriedEvent = next(e for e in events if e.uniqueId() == 'married')
    separatedEvent = next(e for e in events if e.uniqueId() == 'separated')
    divorcedEvent = next(e for e in events if e.uniqueId() == 'divorced')
...
def test_anyMarriedEvents(noEvents):
    marriage = noEvents
    assert marriage.anyMarriedEvents() == False

    Event(parent=marriage, uniqueId='married', dateTime=util.Date(1900, 1, 1))
    assert marriage.anyMarriedEvents() == True


def test_anySeparatedEvents(noEvents):
    marriage = noEvents
    assert marriage.anySeparatedEvents() == False

    Event(parent=marriage, uniqueId='separated', dateTime=util.Date(1900, 1, 1))
    assert marriage.anySeparatedEvents() == True
    

def test_anyDivorcedEvents(noEvents):
    marriage = noEvents
    assert marriage.anyDivorcedEvents() == False

    Event(parent=marriage, uniqueId='divorced', dateTime=util.Date(1900, 1, 1))
    assert marriage.anyDivorcedEvents() == True


def test_everMarried(noEvents):
    assert noEvents.everMarried() == True

    noEvents.setMarried(False)
    assert noEvents.everMarried() == False

    Event(parent=noEvents, uniqueId='married')
    assert noEvents.everMarried() == True
...
def test_everSeparated(noEvents):
    ...
    Event(parent=noEvents, uniqueId='separated')
    assert noEvents.everSeparated() == True
...
def test_everDivorced(noEvents):
    ...
    Event(parent=noEvents, uniqueId='divorced')
    assert noEvents.everDivorced() == True
