...
def test_login_loads_free_diagram(create_ac_mw):
    ac, mw = create_ac_mw()
    assert mw.session.hasFeature(vedana.LICENSE_FREE), "Active features should just be the free license."
    assert mw.serverFileModel.rowCount() == 1, "Free license should only have one file in server file model."
...
def test_login_loads_server_view(test_user_diagrams, test_activation, create_ac_mw):
    ...
    assert mw.serverFileModel.rowCount() == 6, "Server file model hasn't syncronously updated on login."
...
def test_appconfig_tampered_with_should_just_log_out(qtbot, create_ac_mw):
    ...
    tamperedFPath = os.path.join(os.path.abspath(os.path.dirname(__file__)), 'data/cherries_high_sierra')
    ...
    qtbot.clickOkAfter(
        ...
        contains=AppController.S_APPCONFIG_TAMPERED_WITH
    )
...
def test_version_deactivated(test_license, qtbot, create_ac_mw):
    ...
    with mock.patch.object(fdserver_util, 'DEACTIVATED_VERSIONS', da):
        ...
        qtbot.clickOkAfter(
            lambda: ac._pre_event_loop(mw),
            contains=AppController.S_VERSION_DEACTIVATED
        )
