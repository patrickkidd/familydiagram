def main():
    ...
    if not "-s" in sys.argv and not "--server" in sys.argv:
        ...
        parser.add_option(
            ...
            "--attach",
            dest="attach",
            help="Wait for a debugger to attach to this process",
            action="store_true",
            default=False,
        )
        parser.add_option(
        ...
        parser.add_option(
            "-m", "--module", dest="module", help="Run main() in module", default="app"
        )
        parser.add_option(
            "-M",
            "--mainwindow",
            dest="mainwindow",
            action="store_true",
            help="Run main() in module in MainWindow (-m) required",
            default=False,
        )
        parser.add_option(
            "-q",
            "--qml-module",
            dest="qml_module",
            help="Run an isolated qml module",
            default=None,
        )
        parser.add_option(
            "-v",
        ...
        if options.version:
            ...
            spec = importlib.util.spec_from_file_location(
                "version", os.path.join(ROOT, "pkdiagram", "version.py")
            )
