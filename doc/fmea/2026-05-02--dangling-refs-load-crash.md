...
| L2b | Same L2 crash recurs on the **accept** ingress: `PersonalAppController._addCommittedItemsToScene` adds committed extraction items via `scene.addItem` without the L2 resilience, so an Event with an unresolved person ref crashes accept (after diagram save, before cursor signal) | User accepts a PDP whose extraction emitted a structural event referencing a person not committed (FD-319, diagram 1924) | Critical | Personal app log: `AssertionError` at `event.py person()` via `eventsFor`→`marriage.shouldShowFor`; server shows extract 200 + diagram PUT but no `commit-pdp` | **FIXED 2026-05-17** — root cause was normalized-data-flow violation: L2 resilience was wired only into the load ingress (`Scene.read`), not the shared step. `_addCommittedItemsToScene` now calls the same `Scene._dropIrrecoverableEvents` (one shared resilience step for all probabilistic ingresses; see scene/CLAUDE.md "Provenance-normalized scene ingress"). Backend also hardened (FD-319): `validate_pdp_deltas` event positive-ref check + `fix_unresolved_person_refs` + commit orphan-bond skip. |
