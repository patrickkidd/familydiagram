...
// Variant 3C: Horizontal scrollable discussion pills below header
// Quick switching without leaving the view
...
Window {
    ...
    property color pillBg: isDarkMode ? "#3a3a3c" : "#e5e5ea"
    property color pillActiveBg: accentColor
    ...
    Rectangle {
        ...
        // Title - shows diagram name on Discuss tab
        Text {
            anchors.centerIn: parent
            text: currentTab === 0 ? currentDiagram : (currentTab === 1 ? "Learn" : "Plan")
            font.pixelSize: 17
            font.bold: true
            color: textColor
        }
    ...
    // Discussion pills (only on Discuss tab)
    Rectangle {
        id: pillsBar
        anchors.top: header.bottom
        anchors.left: parent.left
        anchors.right: parent.right
        height: currentTab === 0 ? 52 : 0
        color: headerBg
        ...
        ListView {
            ...
            delegate: Rectangle {
                ...
                radius: 18
                color: currentDiscussion === model.name ? pillActiveBg : pillBg
                ...
                Text {
                    ...
                    text: model.name
                    font.pixelSize: 14
                    font.weight: currentDiscussion === model.name ? Font.DemiBold : Font.Normal
                    color: currentDiscussion === model.name ? "white" : textColor
                }

                MouseArea {
                    anchors.fill: parent
                    onClicked: currentDiscussion = model.name
                }
            }
            ...
            footer: Rectangle {
                ...
                MouseArea {
                    anchors.fill: parent
                    onClicked: console.log("New discussion")
                }
            }
        }
    }
    ...
    // Content
    StackLayout {
        anchors.top: pillsBar.visible ? pillsBar.bottom : header.bottom
        anchors.left: parent.left
        anchors.right: parent.right
