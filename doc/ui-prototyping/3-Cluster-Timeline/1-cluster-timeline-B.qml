// Variant B: Colored dots at cluster midpoint
...
Window {
    ...
    Text {
        x: 20; y: 15
        text: "B: Colored Dots at Midpoint"
        font.pixelSize: 14
        font.bold: true
        color: "#ffffff"
    }
    ...
    Text {
        x: 20; y: 35
        text: "Dots at cluster center. Tap to select."
        font.pixelSize: 11
        color: "#909098"
    }
    ...
    Rectangle {
        ...
        Column {
            ...
            Text {
                ...
                text: selectedCluster >= 0 ? clusters[selectedCluster].title : "Tap a dot to select cluster"
                font.pixelSize: 12
    ...
    // Cluster dots at midpoint
    Repeater {
        model: clusters

        Rectangle {
            objectName: "clusterDot_" + index
            property real midYear: (modelData.startYear + modelData.endYear) / 2

            x: xPos(midYear) - (selectedCluster === index ? 7 : 5)
            y: timelineY - (selectedCluster === index ? 32 : 30)
            width: selectedCluster === index ? 14 : 10
            height: selectedCluster === index ? 14 : 10
            radius: width / 2
            color: patternColor(modelData.pattern)
            opacity: selectedCluster === index ? 1.0 : 0.8
    ...
    Text {
        ...
        anchors.horizontalCenter: parent.horizontalCenter
        text: "16 clusters as dots (loses date range info)"
        font.pixelSize: 10
        color: "#606070"
    }
}
