...
// Iteration 7: Taller graph, year markers on x-axis, scroll fix
// Timeline as a story scroll, events are the narrative
// Smooth animations, WCAG compliant
...
Window {
    ...
    // Mini graph properties - TALLER
    property real miniGraphY: 95
    property real miniGraphH: 100  // Increased from 80
    property real gLeft: 50
    property real gWidth: width - 70

    function xPos(year) { return gLeft + ((year - 1920) / 60) * gWidth }
    function yPosMini(val) { return miniGraphY + miniGraphH - ((val + 2) / 6) * miniGraphH }

    // Smooth scroll function - scrolls list to show selected event at top with padding
    function scrollToEvent(idx) {
        if (idx < 0 || idx >= events.length) return

        // Calculate position: all items before idx will be collapsed (100px each)
        var targetY = idx * 100

        // Add small padding from top so user sees context
        targetY = Math.max(0, targetY - 20)

        // Calculate max scroll (all items collapsed except selected one)
        var totalContentHeight = (events.length - 1) * 100 + 200
    ...
    Rectangle {
        ...
        // Mini sparkline graph background
        Rectangle {
            x: 16; y: miniGraphY - 5
            width: parent.width - 32
            height: miniGraphH + 30  // Extra space for year labels
            radius: 10
        ...
        // Event dots on mini graph - CLICKABLE
        Repeater {
            model: events
            Item {
    ...
    ListView {
        id: storyList
        x: 0; y: headerCard.height + 10
        width: parent.width
        height: parent.height - headerCard.height - 60  // Account for legend at bottom
        clip: true
        spacing: 0
        model: events
