...
// Iteration 5: Added clickable graph dots with smooth scroll to event list
...
Window {
    ...
    // Smooth scroll function - scrolls list to show event at index
    function scrollToEvent(idx) {
        if (idx < 0 || idx >= events.length) return
        // Calculate target position: each collapsed item is 100px, expanded is 200px
        var targetY = 0
        for (var i = 0; i < idx; i++) {
            targetY += (selectedEvent === i) ? 200 : 100
        }
        // Center the item in view (offset by ~1/3 of list height)
        targetY = Math.max(0, targetY - storyList.height / 3)
        // Clamp to valid range
        targetY = Math.min(targetY, storyList.contentHeight - storyList.height)
    ...
    Rectangle {
        ...
        // Event dots on mini graph - NOW CLICKABLE
        Repeater {
            model: events
            Item {
