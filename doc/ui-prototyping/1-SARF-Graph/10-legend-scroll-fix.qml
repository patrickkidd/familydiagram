...
// Iteration 10: Legend under graph, scroll adjustment after expand
// For last items that overflow after expansion
...
Window {
    ...
    Timer {
        ...
        onTriggered: {
            if (pendingSelection >= 0) {
                ...
                pendingSelection = -1
                // Start timer to check if we need to scroll more after expand
                scrollAdjustTimer.restart()
            }
        }
    }

    // Timer to adjust scroll after expand animation (250ms) finishes
    Timer {
        id: scrollAdjustTimer
        interval: 300  // After expand animation (250ms)
        onTriggered: {
            if (selectedEvent >= 0) {
    ...
    // Ensure expanded item is fully visible
    function ensureItemVisible(idx) {
        // Calculate where the item ends (top of item + expanded height)
        var itemTop = idx * 100  // All previous items are collapsed
        var itemBottom = itemTop + 200  // Expanded height

        var viewTop = storyList.contentY
        var viewBottom = viewTop + storyList.height

        // If item bottom is below viewport, scroll to show it
        if (itemBottom > viewBottom) {
            var newY = itemBottom - storyList.height
            newY = Math.max(0, newY)
            scrollAdjustAnimation.to = newY
            scrollAdjustAnimation.start()
        }
        // If item top is above viewport, scroll to show it
    ...
    // Header with mini graph and legend
    Rectangle {
        id: headerCard
        width: parent.width
        height: miniGraphY + miniGraphH + 85  // Extra space for legend
        color: card
    ...
    ListView {
        ...
        // Smooth scroll animation for initial scroll
        NumberAnimation {
            id: scrollAnimation
            target: storyList
            property: "contentY"
        ...
        // Adjustment scroll after expand
        NumberAnimation {
            ...
            MouseArea {
                ...
                onClicked: {
                    if (selectedEvent === index) {
                        ...
                        // Check if we need to scroll after expand
                        scrollAdjustTimer.restart()
                    }
                }
            }
