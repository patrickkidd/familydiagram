// Variant A: Zoom into SARF Line Graph
// Cluster bar expands to reveal internal SARF time-series graph
// Events shown as markers on the timeline
import QtQuick 2.15
...
Rectangle {
    ...
    // Hero animation positions
    property real heroStartX: 0
    property real heroStartY: 0
    property real heroStartW: 0
    property real heroStartH: 0

    // SARF colors (Self, Anxiety, Reactivity, Functioning)
    readonly property color sarfS: "#3498db"  // Blue - Self
    readonly property color sarfA: "#e74c3c"  // Red - Anxiety
    readonly property color sarfR: "#9b59b6"  // Purple - Reactivity
    readonly property color sarfF: "#27ae60"  // Green - Functioning
    ...
    property var clusters: [
        ...
        {title: "Work Stress", pattern: "anxiety_cascade", startYear: 2025.41, endYear: 2025.48, row: 2,
         events: [
            {year: 2025.41, label: "Deadline announced"},
            {year: 2025.43, label: "Workload increase"},
            {year: 2025.45, label: "Sleep issues"},
            {year: 2025.47, label: "Burnout signs"},
        ...
        {title: "Family Triangle", pattern: "triangle_activation", startYear: 2025.48, endYear: 2025.53, row: 0,
         events: [
            {year: 2025.48, label: "Initial tension"},
            {year: 2025.49, label: "Side-taking"},
            {year: 2025.50, label: "Conflict peaks"},
            {year: 2025.51, label: "Mediation"},
            {year: 2025.52, label: "Partial resolution"},
            {year: 2025.53, label: "New equilibrium"}
         ],
         sarf: [
        ...
        {title: "Holiday Tension", pattern: "conflict_resolution", startYear: 2025.94, endYear: 2026.02, row: 0,
         events: [
            {year: 2025.94, label: "Planning starts"},
            {year: 2025.95, label: "Disagreement"},
            {year: 2025.96, label: "Family arrives"},
            {year: 2025.97, label: "Tension builds"},
            {year: 2025.98, label: "Argument"},
            {year: 2025.99, label: "Cool down"},
            {year: 2026.00, label: "Apologies"},
            {year: 2026.01, label: "Reconciliation"},
         ...
         sarf: [
            ...
            {year: 2025.99, s: 0.3, a: 0.7, r: 0.6, f: 0.3},
    ...
    SequentialAnimation {
        ...
        ScriptAction {
            script: { focusedClusterIndex = -1; selectedCluster = -1 }
        }
    }
    ...
    Rectangle {
        ...
        // HERO: Expanding cluster with SARF graph inside
        Rectangle {
            id: heroRect
            ...
            color: Qt.darker(focusedClusterIndex >= 0 ? patternColor(clusters[focusedClusterIndex].pattern) : "#404050", 1.5)
            ...
            Rectangle {
                ...
                Text {
                    ...
                    font.pixelSize: 9 + 3 * animProgress
                    font.bold: true
                    color: "white"
                }
            }

            // SARF Line Graph - inside the expanding hero
            Item {
                id: sarfGraph
                anchors.fill: parent
                ...
                function xForYear(year) {
                    return ((year - clusterStart) / clusterSpan) * width
                }
                ...
                // SARF Lines - Canvas drawing
                Canvas {
                    id: sarfCanvas
                    anchors.fill: parent
                    visible: sarfGraph.cluster && sarfGraph.cluster.sarf
                    ...
                    Connections {
                        ...
                        function onAnimProgressChanged() {
                            if (animProgress > 0.3) sarfCanvas.requestPaint()
                        }
                        function onFocusedClusterIndexChanged() {
                            sarfCanvas.requestPaint()
                        }
                    }
                }
                ...
                Repeater {
                    ...
                    Item {
                        ...
                        // Event label on hover/always for now
                        Text {
                            anchors.horizontalCenter: parent.horizontalCenter
                            anchors.top: parent.top
                            anchors.topMargin: 2
                ...
                Column {
                    ...
                    Text { text: "1.0"; font.pixelSize: 7; color: "#505060" }
                    Item { width: 1; height: sarfGraph.height / 2 - 10 }
                    Text { text: "0.5"; font.pixelSize: 7; color: "#505060" }
                    Item { width: 1; height: sarfGraph.height / 2 - 10 }
                    Text { text: "0.0"; font.pixelSize: 7; color: "#505060" }
                }
            }
        }
    }
    ...
    Row {
        ...
        Text {
            text: focusedClusterIndex >= 0 ?
                "May " + Math.round((clusters[focusedClusterIndex].startYear - 2025) * 12) + " → " +
                "May " + Math.round((clusters[focusedClusterIndex].endYear - 2025) * 12)
                : ""
            font.pixelSize: 8
            color: "#606070"
        }
