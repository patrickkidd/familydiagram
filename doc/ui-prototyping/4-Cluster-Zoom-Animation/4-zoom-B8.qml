// Variant B8: Fixed zoom/pan - event bands + SARF lines move together
import QtQuick 2.15
    ...
    Rectangle {
        ...
        Rectangle {
            ...
            Item {
                ...
                // Scrollable/zoomable content container
                Item {
                    id: zoomableContent
                    x: -focusedScrollX
                    y: 0
                    width: contentArea.width * focusedZoom
                    height: contentArea.height

                    // Helper function for x position
                    function xForYear(year) {
                        if (!heroRect.cluster) return 0
                        return ((year - heroRect.clusterStart) / heroRect.clusterSpan) * width
                    }

                    function yForValue(val) {
                        return contentArea.height * (1 - val * 0.85) - 4
                    }
                    ...
                    Repeater {
                        ...
                        Rectangle {
                            ...
                            x: zoomableContent.xForYear(group.year) - (8 + group.events.length * 4)
                            y: 0
                    ...
                    Canvas {
                        ...
                        onPaint: {
                            ...
                            for (var k = 0; k < 4; k++) {
                                ...
                                for (var i = 0; i < data.length; i++) {
                                    var x = zoomableContent.xForYear(data[i].year)
                                    var y = zoomableContent.yForValue(data[i][keys[k]])
                                ...
                                for (i = 0; i < data.length; i++) {
                                    x = zoomableContent.xForYear(data[i].year)
                                    y = zoomableContent.yForValue(data[i][keys[k]])
                                    ctx.beginPath()
                ...
                // Repaint canvas when zoom/scroll changes
                Connections {
                    target: root
                    function onFocusedZoomChanged() { sarfCanvas.requestPaint() }
                    function onFocusedScrollXChanged() { sarfCanvas.requestPaint() }
                    function onFocusedClusterIndexChanged() { sarfCanvas.requestPaint() }
                    function onAnimProgressChanged() { if (animProgress > 0.25) sarfCanvas.requestPaint() }
                }
            }
            ...
            Rectangle {
                ...
                Rectangle {
                    x: {
                        var maxScroll = Math.max(1, contentArea.width * focusedZoom - contentArea.width)
