// Variant C: Slide Reveal
// Detail view slides up from behind the cluster bar
// Like a drawer or sheet expanding from the bar
import QtQuick 2.15
...
Rectangle {
    ...
    // Slide reveal: bar position for anchor point
    ...
    function focusCluster(index, barY) {
    ...
    Rectangle {
                ...
                Repeater {
                    ...
                    Rectangle {
                        ...
                        MouseArea {
                            ...
                            onClicked: {
                                if (isFocused) return
                                focusCluster(index, barDelegate.y)
                            }
                        }
                    }
                }
        ...
        Rectangle {
            id: slidePanel
            visible: focusedClusterIndex >= 0 || animProgress > 0

            anchors.left: parent.left
            anchors.right: parent.right
            anchors.margins: 8

            // Slide up from anchor position
            y: anchorBarY + barHeight + 8 + (8 - anchorBarY - barHeight - 8) * animProgress
            height: (graphHeight - 8) * animProgress
