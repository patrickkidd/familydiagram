// Variant A: Smooth Zoom + Crossfade
// Zooms timeline to center on selected cluster, then crossfades to detail view
// Back button animates reverse (crossfade out, zoom out)
import QtQuick 2.15
...
Rectangle {
    ...
    // Overview zoom/pan state
    property real minZoom: 1.0
    property real maxZoom: 5.0
    property real zoomLevel: 1.0
    property real scrollX: 0

    // Animated zoom/scroll for focus transition
    ...
    function focusCluster(index) {
        ...
        // Calculate target zoom/scroll to center the cluster
        var cluster = clusters[index]
        var clusterMidYear = (cluster.startYear + cluster.endYear) / 2
        var clusterSpan = cluster.endYear - cluster.startYear

        // Zoom to show cluster at ~30% of viewport width
        var desiredClusterWidth = viewportWidth * 0.3
        var clusterFrac = clusterSpan / yearSpan
        targetZoom = Math.min(maxZoom, Math.max(minZoom, desiredClusterWidth / (clusterFrac * baseContentWidth)))

        // Scroll to center cluster
        var clusterCenterX = ((clusterMidYear - minYear) / yearSpan) * baseContentWidth * targetZoom
        targetScrollX = Math.max(0, clusterCenterX - viewportWidth / 2)
        var maxScroll = Math.max(0, baseContentWidth * targetZoom - viewportWidth)
    ...
    // Focus animation: zoom/scroll then crossfade
    SequentialAnimation {
        id: focusAnim
    ...
    // Unfocus animation: crossfade then zoom out
    SequentialAnimation {
        id: unfocusAnim
    ...
    Rectangle {
        ...
        PinchArea {
            ...
            // ============ FOCUSED VIEW (crossfades in) ============
            Item {
                anchors.fill: parent
                opacity: animProgress
                visible: animProgress > 0

                // Focused cluster detail view
                Column {
                    anchors.fill: parent
                    anchors.margins: 8
                    spacing: 8
                    ...
                    Row {
                        ...
                        Column {
                            ...
                            Text {
                                text: focusedClusterIndex >= 0 ? clusters[focusedClusterIndex].events + " events" : ""
                                font.pixelSize: 10
                                color: "#909098"
                            }
                        }
