// Variant B5: Combined B2 legend + B4 line graph
// - No axis gridlines (aesthetic flow only)
// - Same-day events handled with slight x-offset
// - Pinch-to-zoom in focused view
import QtQuick 2.15
...
Rectangle {
    ...
    // Data model: events with SARF shifts, multiple events can share same day
    property var clusters: [
        {title: "Race Committee", pattern: "conflict_resolution", startYear: 2025.37, endYear: 2025.42, row: 0,
         data: [
            ...
            {year: 2025.38, label: "Sleepless night", s: 0.4, a: 0.7, r: 0.7, f: 0.45}, // same day
        ...
        {title: "Work Stress", pattern: "anxiety_cascade", startYear: 2025.41, endYear: 2025.48, row: 2,
         data: [
            ...
            {year: 2025.41, label: "Boss criticism", s: 0.65, a: 0.45, r: 0.4, f: 0.6}, // same day
            {year: 2025.43, label: "Workload increase", s: 0.5, a: 0.5, r: 0.5, f: 0.5},
            {year: 2025.45, label: "Sleep issues", s: 0.3, a: 0.7, r: 0.7, f: 0.3},
            {year: 2025.45, label: "Headaches start", s: 0.25, a: 0.75, r: 0.7, f: 0.28}, // same day
        ...
        {title: "Holiday Tension", pattern: "conflict_resolution", startYear: 2025.94, endYear: 2026.02, row: 0,
         data: [
            ...
            {year: 2025.97, label: "Old conflict resurfaces", s: 0.28, a: 0.75, r: 0.85, f: 0.28}, // same day
    ...
    Text {
        x: 12; y: 4
        text: "B5: Combined + Pinch Zoom"
        font.pixelSize: 10
        font.bold: true
        color: "#808090"
    }

    // Pattern legend (overview) - from B2
    Row {
        x: 12; y: 18
        spacing: 10
    ...
    // SARF legend (focused) - line style from B2
    Row {
        x: 12; y: 18
        spacing: 10
    ...
    Rectangle {
        ...
        Rectangle {
            ...
            PinchArea {
                ...
                // SARF Graph - pure aesthetic flow, no gridlines
                Item {
                    id: sarfGraph
                    anchors.fill: parent
                    ...
                    function xForYear(year) {
                        var baseW = width
                        var x = ((year - heroRect.clusterStart) / heroRect.clusterSpan) * baseW * focusedZoom - focusedScrollX
                        return x
                    }
                    function yForValue(val) { return height * (1 - val * 0.85) - 4 }

                    // SARF Canvas - smooth lines only
                    Canvas {
                        anchors.fill: parent
                        ...
                        onPaint: {
                            ...
                            // Draw smooth lines - thicker for aesthetic
                            for (var k = 0; k < 4; k++) {
                                ...
                                for (var i = 0; i < data.length; i++) {
                                    ...
                                    if (i === 0) {
                                        ...
                                        var prevX = sarfGraph.xForYear(data[i-1].year)
                                        var prevY = sarfGraph.yForValue(data[i-1][keys[k]])
                                        var cpX = (prevX + x) / 2
                ...
                Item {
                    ...
                    // Group events by day for same-day handling
                    property var eventGroups: {
                        if (!heroRect.cluster) return []
                        var data = heroRect.cluster.data
                        var groups = []
                        var currentYear = -1
                        var currentGroup = []
                        ...
                        for (var i = 0; i < data.length; i++) {
                            if (Math.abs(data[i].year - currentYear) < 0.001) {
                    ...
                    Repeater {
                        model: eventLabels.eventGroups

                        Item {
                            property var group: modelData
                            property real baseX: eventLabels.xForYear(group[0].year)
                            x: baseX
                            width: 1

                            // Stacked dots for same-day events
                            Column {
                                anchors.horizontalCenter: parent.horizontalCenter
                                y: -4
                                spacing: -4
                            ...
                            // Combined label (first event, count if multiple)
                            Text {
                                anchors.horizontalCenter: parent.horizontalCenter
                                y: 6
                                text: group.length > 1 ? group[0].label + " +" + (group.length - 1) : group[0].label
                                font.pixelSize: 6
                                color: "#707080"
                                width: 45
                                horizontalAlignment: Text.AlignHCenter
