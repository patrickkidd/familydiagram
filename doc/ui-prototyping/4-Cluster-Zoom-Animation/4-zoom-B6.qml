// Variant B6: Same-day events spread on x-axis
// - Events on same day get slight x-offset for visual clarity
// - Thinner, more elegant lines
// - Hover-like interaction hints
import QtQuick 2.15
...
Rectangle {
    ...
    // Pre-process data to add x-offsets for same-day events
    function preprocessData(data) {
        var result = []
        var lastYear = -1
        var sameCount = 0
        var daySpread = 0.003 // spread same-day events by this amount

        for (var i = 0; i < data.length; i++) {
            var item = JSON.parse(JSON.stringify(data[i]))
            if (Math.abs(item.year - lastYear) < 0.001) {
                sameCount++
                item.displayYear = item.year + sameCount * daySpread
            } else {
                sameCount = 0
    ...
    Text {
        x: 12; y: 4
        text: "B6: X-Spread Same-Day Events"
        font.pixelSize: 10
        font.bold: true
        color: "#808090"
    }
    ...
    Rectangle {
        ...
        Rectangle {
            ...
            property var processedData: cluster ? preprocessData(cluster.data) : []
            ...
            PinchArea {
                ...
                Item {
                    ...
                    Canvas {
                        ...
                        onPaint: {
                            ...
                            if (!heroRect.processedData || heroRect.processedData.length === 0) return

                            var data = heroRect.processedData
                            var colors = [sarfS.toString(), sarfA.toString(), sarfR.toString(), sarfF.toString()]
                            var keys = ['s', 'a', 'r', 'f']

                            // Draw lines with slight curve
                            for (var k = 0; k < 4; k++) {
                                ...
                                for (var i = 0; i < data.length; i++) {
                                    var x = sarfGraph.xForYear(data[i].displayYear || data[i].year)
                                ...
                                for (i = 0; i < data.length; i++) {
                                    x = sarfGraph.xForYear(data[i].displayYear || data[i].year)
                ...
                // Event labels - each event gets its own label
                Item {
                    id: eventLabels
                    anchors.left: parent.left
                    anchors.right: parent.right
                    ...
                    Repeater {
                        model: heroRect.processedData

                        Item {
                            x: eventLabels.xForYear(modelData.displayYear || modelData.year)
                            width: 1
